    ...
    describe Item do
      [nil, true, false, 1234, 0.16, 'ok'].each do |value|
        it 'require name to be a non-empty string' do
          subject.name = value
          expect(subject).not_to be_valid, "Expected '#{value}' to be invalid for name of the Item"
        end
      end
    end
    ...
    describe Invoice do
      ...
      [nil, true, false, 0.16, 'ok'].each do |value|
        it 'require payment_terms to be a non-empty string' do
          ...
          expect(invoice).not_to be_valid, "Expected '#{value}' to be invalid for payment_terms Invoice"
        end
      end

      [nil].each do |value|
        it 'require payment_due_date to be a non-empty string' do
          invoice.payment_due_date = value
          expect(invoice).not_to be_valid, "Expected '#{value}' to be invalid for payment_due_date Invoice"
        end
      end
      ...
      [nil, 0.16, true, false, 'ok', 'invalid_status'].each do |value|
        it 'require status to be a non-empty string' do
          ...
          expect(invoice).not_to be_valid, "Expected '#{value}' to be invalid for status Invoice"
        end
      end
    end
